...
// connection_query.query("insert into mysql_first set ?", post, function (err, rows) {
//     // console.log(err)
//     console.log(rows);
// });
// console.log(connection_query.sql);
// var sorter = 'name';
// console.log(connection_query.escapeId(sorter))
// var sql    = 'SELECT * FROM mysql_first ORDER BY ' + connection_query.escapeId(sorter);
// connection_query.query(sql, function(err, results) {
//
// });
// 标识符  （数据库名、表名、列名）等
// let columns = ['id','name','email']
// connection_query.query("select ?? from mysql_first where id=?",[columns,4],function(err,rows){
//     console.log(rows)
// })
var sql = "SELECT * FROM ?? WHERE ?? = ?";
var inserts = ['users', 'id', 2];
sql = connection_query.format(sql, inserts);
console.log(sql)


connection_query.end(function (err) {
    if (err) {
        console.log(err);
    }
});
